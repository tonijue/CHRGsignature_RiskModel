...
# make a list of patient ID's with just IDHwt or IDHm
#IDHwt_glioma <- glioma.pheno[glioma.pheno$IDH.status == "WT", ] #there shoould only be 538 samples with IDH WT status
#IDHwt_glioma <- IDHwt_glioma %>% drop_na(Patient_ID) # remove rows with NA in Patient_ID, replace - with .

# replace - with . in surv file to match the colnames in the counts dataframe.
#tmp <- str_replace_all(IDHwt_glioma$Patient_ID, "-", ".")
#IDHwt_glioma$Patient_ID <- tmp

#IDHwt_ptID <- as.character(IDHwt_glioma$Patient_ID) 

#glioma.IDHwt.log2.copper <- glioma.log2.coxdata.unique %>% filter(row.names(glioma.log2.coxdata.unique) %in% IDHwt_ptID, )
#glioma.IDHwt.log2.copper <- glioma.log2.coxdata.unique[row.names(glioma.log2.coxdata.unique) %in% IDHwt_ptID,]
#> dim(glioma.IDHwt.log2.copper)
#[1] 237  52
#237 out of 538 samples with IDHwt had data for cox analysis.

#save(glioma.IDHwt.log2.copper, file = "/Users/tonijue/Documents/Copper in glioma/Copper in glioma/TCGA LGG 20220523/Scripts/Version 3_2023/10 20231108_gliomaIDHwt_log2_coxdata_limma-voom.Rdata")

#IDHm_glioma <- glioma.pheno[glioma.pheno$IDH.status == "Mutant", ]
#IDHm_glioma <- IDHm_glioma %>% drop_na(Patient_ID)

# replace - with . in surv file to match the colnames in the counts dataframe.
#tmp <- str_replace_all(IDHm_glioma$Patient_ID, "-", ".")
#IDHm_glioma$Patient_ID <- tmp

#IDHm_ptID <- as.character(IDHm_glioma$Patient_ID)

#glioma.IDHm.log2.copper <- glioma.log2.coxdata.unique %>% filter(row.names(glioma.log2.coxdata.unique) %in% IDHm_ptID, )
#glioma.IDHm.log2.copper <- glioma.log2.coxdata.unique[row.names(glioma.log2.coxdata.unique) %in% IDHm_ptID,]
#> dim(glioma.IDHm.log2.copper)
#[1] 420  52
#420 out of 457 IDHm samples had data for cox analysis.
...
#tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_train_log2counts - lasso_path_plot.tiff",
...
# open a TIFF device to save the plot####
#tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_train_log2counts - lasso_path_plot-lambda.tiff", width = 8, height = 6, units = "in", res = 300)
...
# Cross-validation for tuning parameter screening in the LASSO regression model. KEEP UNCHANGED FOR VALIDATION.  THIS IS CONSTANT BASED ON TRAINING SET.
#set.seed(100)
...
# open a TIFF device to save the plot ####
#tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_train_log2counts - partial-likelihood-deviance.tiff", width = 8, height = 6, units = "in", res = 300)
...
ggsave("OP/Pipeline OP5/TCGA Test OP/Images/TCGA_test_log2counts - ROC.tiff", plot = last_plot(), width = 900/72, height = 900/72, dpi = 300)
...
ggsave("OP/Pipeline OP5/TCGA Test OP/Images/TCGA_test_log2counts - KM Risk Score.tiff", plot = plot_combined, width = 900/72, height = 900/72, dpi = 300)
...
save(exp_dat, file="OP/Pipeline OP5/TCGA Test OP/TCGA_test_log2counts - exp_dat.RData")
...
# Save the combined plot ####
ggsave("OP/Pipeline OP5/TCGA Test OP/Images/TCGA_test_log2counts - distribution and heatmap.tiff", plot = combined, width = 1100/72, height = 751/72, dpi = 300)
...
pheno.coxdata <- pheno.coxdata[,-(56:57)] # removes duplicate OS and OS.time columns
...
new_dat_cox$Age_years <- as.numeric(new_dat_cox$Age_years)
...
save(new_dat_cox, file = "OP/Pipeline OP5/TCGA Test OP/TCGA_test_log2counts - new_dat_cox.RData")

write.csv(new_dat_cox,'OP/Pipeline OP5/TCGA Test OP/CSV/TCGA_test_log2counts - new_dat_cox.csv',row.names = FALSE)
...
###################### keep below unchanged from train set ######################
#dd <- datadist(nom_dat_cox) # KEEP CONSTANT?

#save(dd, file="OP/Pipeline OP5/Train OP/TCGA_train_log2countts - dattadist.RData")
...
# plot nomogram ####
#tiff(filename = "OP/Pipeline OP5/Images/TCGA_train_log2counts - Nomogram.tiff",
...
# open a TIFF device to save the plot
tiff(filename = "OP/Pipeline OP5/TCGA Test OP/Images/TCGA_test_log2counts - Nomogram Calibration.tiff",
     width = 10, height = 10, units = "in", res = 300)
...
ggsave("OP/Pipeline OP5/TCGA Test OP/Images/TCGA_test_log2counts - Nomogram ROC.tiff", plot = last_plot(), width = 900/72, height = 751/72, dpi = 300)

new_tcga_nom <- nom_dat_cox

save(new_tcga_nom, file = 'OP/Pipeline OP5/TCGA Test OP/TCGA_test_log2counts - new_tcga_nom.Rdata')
