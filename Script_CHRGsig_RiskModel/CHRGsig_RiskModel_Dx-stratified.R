...
# This pipeline is for analysis each histology separately.
...
# aggregate duplicate patient ID's (optional)
# gives you a data frame with a list of ids and the number of times they occurred.
#n_occur <- data.frame(table(glioma.train.log2.coxdata$X_PATIENT))
#> nrow(dups)
#[1] 11

# tells you which ids occurred more than once. (optional)
#dups <- n_occur[n_occur$Freq > 1,] # 14 patient IDs duplicated

# returns the records with more than one occurrence. (optional)
#dups.rec.trainlog2 <- glioma.train.log2.coxdata[glioma.train.log2.coxdata$X_PATIENT %in% n_occur$Var1[n_occur$Freq > 1],] # 23 samples withe repeating occurrences

# gives you a data frame with a list of ids and the number of times they occurred.
#n_occur <- data.frame(table(glioma.test.log2.coxdata$X_PATIENT))
#> nrow(dups)
#[1] 6
...
# returns the records with more than one occurrence. (optional)
#dups.rec.testlog2 <- glioma.test.log2.coxdata[glioma.test.log2.coxdata$X_PATIENT %in% n_occur$Var1[n_occur$Freq > 1],] # 13 samples withe repeating occurrences
...
# create an IDHwt and IDHm subdataframes from glioma.log2.coxdata.unique for validation (optional)
dim(glioma.log2.coxdata.unique)
# [1] 666  52

dim(clean.WHO.CNS5_v2)
# [1] 1099   35

# Merge glioma.log2.coxdata.unique and clean.WHO.CNS5_v2 so we can subset samples based on histology.
glioma.log2.coxdata.pheno <- merge(glioma.log2.coxdata.unique, clean.WHO.CNS5_v2, by = 0)
glioma.log2.coxdata.pheno <- column_to_rownames(glioma.log2.coxdata.pheno, var="Row.names")
dim(glioma.log2.coxdata.pheno)
# [1] 664  87

save(glioma.log2.coxdata.pheno, file = "OP/Pipeline OP5/glioma.log2.coxdata.pheno.Rdata")

# make a list of patient ID's for each WHO CNS5 Histology
table(glioma.log2.coxdata.pheno$WHO_CNS5_histology, useNA="ifany")
#Astrocytoma      Glioblastoma Oligodendroglioma           unknown 
#        225               211               148                80 

table(glioma.log2.coxdata.pheno$WHO_CNS_diagnosis.., useNA="ifany")
# Astrocytoma, IDH-mutant                      Glioblastoma, IDH-wildtype 
#                     225                                             211 
# Oligodendroglioma, IDH-mutant, 1p/19q-codeleted            Unclassified 
#                                             148                      80 

cols_to_keep <- c(copper.DEGlist, "Gender", "Age_years", "OS.x", "OS.time.x", "WHO_CNS5_diagnosis", "WHO_CNS_diagnosis..","WHO_CNS5_histology",  "WHO_CNS5_grade", "WHO_CNS5_IDHstatus", "WHO_CNS_1p.19q._codeletion", "MGMT.promoter.status.x")

tmp <- glioma.log2.coxdata.pheno[, cols_to_keep]

Astro <- tmp[tmp$WHO_CNS_diagnosis.. == "Astrocytoma, IDH-mutant", ] 
Oligo <- tmp[tmp$WHO_CNS_diagnosis.. == "Oligodendroglioma, IDH-mutant, 1p/19q-codeleted", ] 
GBM <- tmp[tmp$WHO_CNS_diagnosis.. == "Glioblastoma, IDH-wildtype", ] 

full_tcga <- glioma.log2.coxdata.pheno[, cols_to_keep]
...
y <- data.matrix(Surv(coxdata$OS.time.x,coxdata$OS.x)) 
...
# 0.9227908 0.7535578 0.6362022 - Astrocytoma
# 0.8160097 0.8120994 0.8828835 - Oligo
# 0.6471275 0.8054594 0.8827796 - GBM
...
ggsave("OP/Pipeline OP5/Train OP/Images/TCGA_Astrocytoma_log2counts - ROC.tiff", plot = last_plot(), width = 900/72, height = 900/72, dpi = 300)

ggsave("OP/Pipeline OP5/Train OP/Images/TCGA_Oligodendroglioma_log2counts - ROC.tiff", plot = last_plot(), width = 900/72, height = 900/72, dpi = 300)

ggsave("OP/Pipeline OP5/Train OP/Images/TCGA_GBM_log2counts - ROC rerun-28082024.tiff", plot = last_plot(), width = 900/72, height = 900/72, dpi = 300)
...
fit.surv <- Surv(time = coxdata$OS.time.x, event = coxdata$OS.x)
km <- survfit(fit.surv ~ 1, data = coxdata)
km_2 <- survfit(fit.surv ~ Risk.Score.Level, data = coxdata)

# Not needed for GBM since Risk Group is only High.
dat.survdiff <- survdiff(Surv(OS.time.x,OS.x) ~ Risk.Score.Level, data = coxdata) 
...
plot <- 
  ggsurvplot(km_2,
             ...
             legend.labs = c("High"),# change to c("High", "Low"), for Astro and Oligo
             font.tickslab = c(18),
             linetype =  c("solid"),# change to c("solid", "dashed"), for Astro and Oligo
             xlab="Days",
             ylab="Overall Survival",
             pval=F, # TRUE for Astrocytoma and Oligo, FALSe for GBM
             pval.method=F, # TRUE for Astrocytoma and Oligo, FALSe for GBM
             #pval.size = 8,
             #pval.coord = c(0, 0.03),
             ...
             fontsize = 5.5,
             palette= c("High" = "#ED0000FF"), # change to c("High" = "#ED0000FF", "Low" = "#00468BFF"), for Astro and Oligo
...
ggsave("OP/Pipeline OP5/Train OP/Images/TCGA_Astrocytoma_log2counts - KM Risk Score.tiff", plot = plot_combined, width = 900/72, height = 900/72, dpi = 300)

ggsave("OP/Pipeline OP5/Train OP/Images/TCGA_Oligodendroglioma_log2counts - KM Risk Score.tiff", plot = plot_combined, width = 900/72, height = 900/72, dpi = 300)

ggsave("OP/Pipeline OP5/Train OP/Images/TCGA_GBM_log2counts - KM Risk Score rerun-29082024.tiff", plot = plot_combined, width = 900/72, height = 900/72, dpi = 300)
...
sur_dat <- data.frame(s = 1:length(lasso.prob[ ,1]),
                      t = coxdata[names(sort(lasso.prob[ ,1])), 'OS.time.x'],
                      OS = coxdata[names(sort(lasso.prob[ ,1])), 'OS.x'])
...
save(exp_dat, file="OP/Pipeline OP5/Train OP/TCGA_Astrocytoma_log2counts - exp_dat.RData")
save(exp_dat, file="OP/Pipeline OP5/Train OP/TCGA_Oligodendroglioma_log2counts - exp_dat.RData")
save(exp_dat, file="OP/Pipeline OP5/Train OP/TCGA_GBM_log2counts - exp_dat.RData")
...
cols_to_keep <- c(copper.DEGlist, "OS.x", "OS.time.x", "Gender", "Age (Years)", "Age", "WHO_CNS_diagnosis..", "Grade", "MGMTp.Status", "Risk.Score", "Risk.Score.Level")

tmp <- new_dat_cox[, cols_to_keep]

new_dat_cox <- tmp
new_dat_cox <- column_to_rownames(new_dat_cox, var="Row.names")

save(new_dat_cox, file = "OP/Pipeline OP5/Train OP/TCGA_Astrocytoma_log2counts - new_dat_cox.RData")
save(new_dat_cox, file = "OP/Pipeline OP5/Train OP/TCGA_Oligodendroglioma_log2counts - new_dat_cox.RData")
save(new_dat_cox, file = "OP/Pipeline OP5/Train OP/TCGA_GBM_log2counts - new_dat_cox.RData")

write.csv(new_dat_cox,'OP/Pipeline OP5/Train OP/CSV/TCGA_Astrocytoma_log2counts - new_dat_cox.csv',row.names = FALSE)
write.csv(new_dat_cox,'OP/Pipeline OP5/Train OP/CSV/TCGA_Oligodendroglioma_log2counts - new_dat_cox.csv',row.names = FALSE)
write.csv(new_dat_cox,'OP/Pipeline OP5/Train OP/CSV/TCGA_GBM_log2counts - new_dat_cox.csv',row.names = FALSE)
...
print(sapply(new_dat_cox, function(x) sum(is.na(x)))) # Check for NAs
...
# do this for training set - this is needed to create the nomogram
#dd <- datadist(new_dat_cox)
#options(datadist="dd")
#nom_cox <- cph(Surv(OS.time,OS) ~ Grade + age_level2 + Risk_Score_level2, data = new_dat_cox, x = T, y = T, surv = T)
#nom_cox <- cph(Surv(OS.time.x,OS.x) ~ Grade + Age + Risk.Score.Level, data = new_dat_cox, x = T, y = T, surv = T)
...
# C-index
f <- coxph(Surv(OS.time.x,OS.x) ~ Grade + Age + Risk.Score.Level, data = new_dat_cox)
...
# 0.74205915 0.04049316 - Astrocytoma
# 0.78113879 0.05943967 - Oligo
# C-index could not be calculated for GBM because it was all Grade 4 and all High Risk. contrasts can be applied only to factors with 2 or more levels. No point calculating this if it's only age that will be considered.

# timeROC for nomogram model
pred_nom <- predict(f, newdata = new_dat_cox)
new_dat_cox$pred_nom <- as.numeric(pred_nom)
...
# 0.9217605 0.7916470 0.6643136 - Astro
# 0.6471275 0.8054594 0.8827796 - Oligo
...
ggsave("OP/Pipeline OP5/Train OP/Images/TCGA_Astrocytoma_log2counts - Nomogram ROC.tiff", plot = last_plot(), width = 900/72, height = 751/72, dpi = 300)
ggsave("OP/Pipeline OP5/Train OP/Images/TCGA_Oligodendroglioma_log2counts - Nomogram ROC.tiff", plot = last_plot(), width = 900/72, height = 751/72, dpi = 300)

new_tcga_nom <- nom_dat_cox

save(new_tcga_nom, file = 'OP/Pipeline OP5/Train OP/TCGA_Astrocytoma_log2counts - new_tcga_nom.Rdata')
save(new_tcga_nom, file = 'OP/Pipeline OP5/Train OP/TCGA_Oligodendroglioma_log2counts - new_tcga_nom.Rdata')

# merge clinical full_tcga_new_dat df ######
# adding clinical hx to heatmap
load("~/Documents/Copper in glioma/Copper in glioma/TCGA LGG 20220523/Scripts/Version 3_2023/Version 3_2023_rebuttal/OP/Pipeline OP5/Train OP/full_tcga_exp_dat.RData")
dim(full_tcga_exp_dat)
# [1] 584  19

load("~/Documents/Copper in glioma/Copper in glioma/TCGA LGG 20220523/Scripts/Version 3_2023/Version 3_2023_rebuttal/OP/Pipeline OP5/Train OP/full_tcga_new_dat_cox.RData")
dim(full_tcga_new_dat)
# [1] 584  60
names(full_tcga_new_dat)

load("TCGA_summarized_intervention.Rdata")
dim(summarised_TCGA_intervention)
# [1] 474   6
names(summarised_TCGA_intervention)

# Merge clinical intervention
tmp <- summarised_TCGA_intervention

tmp$bcr_patient_barcode <- str_replace_all(tmp$bcr_patient_barcode, "-", ".")
  
tmp <- column_to_rownames(tmp, var="bcr_patient_barcode")
full_new_tcga_nom <- merge(full_tcga_new_dat, tmp, by=0, all.x=TRUE, all.y=FALSE)

full_new_tcga_nom <- column_to_rownames(full_new_tcga_nom, var="Row.names")

# Add other clinical info in full_new_tcga_nom
summarized_tx_sched$bcr_patient_barcode <- str_replace_all(summarized_tx_sched$bcr_patient_barcode, "-", ".")
summarized_disease_stat$bcr_patient_barcode <- str_replace_all(summarized_disease_stat$bcr_patient_barcode, "-", ".")

# check for common patient IDs
v1 <- full_new_tcga_nom$Row.names
v2 <- summarized_tx_sched$bcr_patient_barcode
v3 <- summarized_disease_stat$bcr_patient_barcode
...
# Merge DFs
tmp <- column_to_rownames(tmp, var="bcr_patient_barcode")
full_new_tcga_nom2 <- merge(full_new_tcga_nom, summarized_tx_sched, by.x="Row.names", by.y="bcr_patient_barcode", all.x=TRUE, all.y=FALSE)
full_new_tcga_nom3 <- merge(full_new_tcga_nom2, summarized_disease_stat, by.x="Row.names", by.y="bcr_patient_barcode", all.x=TRUE, all.y=FALSE)

full_new_tcga_nom3 <- column_to_rownames(full_new_tcga_nom3, var="Row.names")


save(full_new_tcga_nom, file = 'OP/Pipeline OP5/Train OP/full_tcga_new_tcga_nom.Rdata')
save(full_new_tcga_nom3, file = 'OP/Pipeline OP5/Train OP/full_tcga_new_tcga_nom3.Rdata')

########## HEATMAP WITH CLINICAL ANNOTATIONS ##########################

exp_dat <- full_tcga_exp_dat

tmp = t(scale(exp_dat))
tmp[tmp > 1] = 1
tmp[tmp < -1] = -1

df <- as.data.frame(full_new_tcga_nom)
...
df <- df %>% 
  dplyr::rename(`Risk Group` = Risk.Score.Level,
                ...
                `Prior glioma` = prior_glioma,
                `Primary outcome` = primary_therapy_outcome_success,
                #`1p/19q Codel Status` = Codel.Status,
                `WHO CNS5` = WHO_CNS_diagnosis.., # fix for Tcga set
                #`IDHm Status` = IDHm.Status
  )

#### Generate column annotations - Astrocytoma ####
annotation <- df[ , c('OS.x', 'Gender', 'Age', "Prior glioma", "Intervention", 'Grade', "MGMTp Status", 'Risk Group', "WHO CNS5")]
#annotation <- df[ , c('OS.x', 'Gender', 'Age', 'Grade', "MGMTp Status", 'Risk Group', "WHO CNS5")]
...
annotation$`Prior glioma` <- droplevels(annotation$`Prior glioma`)
annotation$`Prior glioma` <- as.character(annotation$`Prior glioma`)

str(annotation)

ann_colors1 <- list(
  `WHO CNS5` = c("Astrocytoma, IDH-mutant" = "#0099B4FF",
                 "Oligodendroglioma, IDH-mutant, 1p/19q-codeleted" = "#925e9f",
                 "Glioblastoma, IDH-wildtype" = "#42b540"),
  ...
  Grade = c("Grade 2" = "#00468B99", 
            "Grade 3" = "#ED000099", 
            "Grade 4" = "#42B54099"),
  Intervention = c("Chemotherapy only" = "#42B54099",
                   "RT" = "#FDAF9199",
                   "RT + Chemotherapy only" = "#1B191999",
                   "Chemotherapy + Other Tx" = "#ED000099",
                   "Other Tx" = "#3C5488B2"),
  `Prior glioma` = c("NO" = "#0099B4FF",
                     "YES" = "#925e9f"),
...
# open a TIFF device to save the plot 
tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_Astrocytoma_log2counts - clinical heatmap output no cluster.tiff",
     width = 12, height = 10, units = "in", res = 300)
print(heatmap_plot)
dev.off()

# open a TIFF device to save the plot 
tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_Oligodendroglioma_log2counts - clinical heatmap output 2 cluster.tiff",
     width = 12, height = 10, units = "in", res = 300)
print(heatmap_plot)
dev.off()

# open a TIFF device to save the plot 
tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_GBM_log2counts - clinical heatmap output TEST cluster.tiff",
     width = 12, height = 10, units = "in", res = 300)
print(heatmap_plot)
dev.off()

############### create heatmap for all samples using new tcga df
astro_exp_dat <- exp_dat
dim(astro_exp_dat)
# [1] 225  19

oligo_exp_dat <- exp_dat
dim(oligo_exp_dat)
# [1] 148  60
...
full_tcga_exp_dat <- bind_rows(astro_exp_dat,oligo_exp_dat,gbm_exp_dat)
dim(full_tcga_exp_dat)
#[1] 584  19
save(full_tcga_exp_dat, file="OP/Pipeline OP5/Train OP/full_tcga_exp_dat.RData")

astro_new_dat <- new_dat_cox
dim(astro_new_dat)
# [1] 225  60

oligo_new_dat <- new_dat_cox
dim(oligo_new_dat)
# [1] 148  60

gbm_new_dat <- new_dat_cox
dim(gbm_new_dat)
# [1] 211  60

full_tcga_new_dat <- bind_rows(astro_new_dat,oligo_new_dat,gbm_new_dat)
dim(full_tcga_new_dat)
# [1] 584  60
save(full_tcga_new_dat, file="OP/Pipeline OP5/Train OP/full_tcga_new_dat_cox.RData")

tmp = t(scale(full_tcga_exp_dat))
tmp[tmp > 1] = 1
tmp[tmp < -1] = -1

df <- as.data.frame(full_tcga_new_dat) 
...
annotation <- df[ , c('OS.x', 'Gender', 'Age', "MGMTp Status", 'Grade', 'Risk Group', 'WHO CNS5')] 
...
ann_colors <- list(
  ...
  Gender = c(female = "#E64B35B2", 
             male = "#3C5488B2"),
  `WHO CNS5` = c("Glioblastoma, IDH-wildtype" = "#42b540",
                                "Astrocytoma, IDH-mutant" = "#0099b4",
                                "Oligodendroglioma, IDH-mutant, 1p/19q-codeleted" = "#925e9f"),
  Grade = c("Grade 2" = "#00468B99", 
            "Grade 3" = "#ED000099", 
            "Grade 4" = "#42B54099"),
...
heatmap_plot <-
  pheatmap(tmp, 
           ...
           margins = c(10, 100, 10, 10),  # top, right, bottom, left margins
           ...
           # Add a gap to the right of the heatmap
           gaps_col = ncol(tmp),
           
           # Adjust legend position
           #legend_position = "bottom",
           
           # Adjust annotation legend position
           annotation_legend_side = "bottom"
  )

print(heatmap_plot)

# open a TIFF device to save the plot - full TCGA with intervention
tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_full_log2counts - clinical heatmap output with intervention.tiff",
     width = 16, height = 11, units = "in", res = 300)
print(heatmap_plot)
dev.off()


# open a TIFF device to save the plot - TRAIN SET
tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_full_log2counts - clinical heatmap output 3col cluster.tiff",
...
count_365_or_less <- sum(new_dat_cox$OS.time.x <= 365, na.rm = TRUE)
count_366_to_1095 <- sum(new_dat_cox$OS.time.x > 365 & new_dat_cox$OS.time.x <= 1095, na.rm = TRUE)
count_1096_or_more <- sum(new_dat_cox$OS.time.x >= 1096, na.rm = TRUE)
...
#"/Users/tonijue/Documents/Copper in glioma/Copper in glioma/TCGA LGG 20220523/Scripts/Version 3_2023/Version 3_2023_rebuttal/TCGA_drug_regimen_sched.csv"

#"/Users/tonijue/Documents/Copper in glioma/Copper in glioma/TCGA LGG 20220523/Scripts/Version 3_2023/Version 3_2023_rebuttal/TCGA_drug_disease_status.csv"

names(full_new_tcga_nom3)

strat_analysis <- full_new_tcga_nom3[, c(53, 55:58, 60:61, 63:65, 67, 69)] # keep only clinical variables
names(strat_analysis)

strat_analysis <- strat_analysis %>% 
  dplyr::rename(`Risk Group` = Risk.Score.Level,
                `MGMTp Status` = MGMTp.Status,
                `WHO CNS5` = WHO_CNS_diagnosis.., 
                `Prior glioma` = prior_glioma,
                `Primary Therapy Outcome` = primary_therapy_outcome_success
  )
names(strat_analysis)



# rename elements in strat_analysis$WHO CNS5
strat_analysis <- strat_analysis %>%
  mutate(`WHO CNS5` = case_when(
    `WHO CNS5` == "Astrocytoma, IDH-mutant" ~ "Astrocytoma",
    `WHO CNS5` == "Glioblastoma, IDH-wildtype" ~ "Glioblastoma",
    `WHO CNS5` == "Oligodendroglioma, IDH-mutant, 1p/19q-codeleted" ~ "Oligodendroglioma",
    TRUE ~ `WHO CNS5`  # Keep the original value if no condition is met
  ))

# apply as.factor to all column so that we can use summary()
cols <- colnames(strat_analysis)
strat_analysis[cols] <- lapply(strat_analysis[cols], factor)

summary(strat_analysis)

# Create stack bar graphs to recreate what you have in the paper.
library("ggsci")

names(strat_analysis)
#[1] "Gender"                  "Age"                     "WHO CNS5"                "Grade"                  
#[5] "MGMTp Status"            "Risk Group"              "Radiotherapy"            "Intervention"           
#[9] "Prior glioma"            "Primary Therapy Outcome" "Regimen"                 "Disease Status"

# Assuming clinvar is a column in strat_analysis
clinvar <- "Gender"
clinvar <- "Age"
clinvar <- "WHO CNS5"
clinvar <- "Grade"
clinvar <- "MGMTp Status"
clinvar <- "Radiotherapy"
clinvar <- "Intervention"
clinvar <- "Prior glioma"
clinvar <- "Primary Therapy Outcome"
clinvar <- "Regimen"
clinvar <- "Disease Status"

# First, filter out NA values and calculate percentages
tmp <- strat_analysis %>%
  filter(!is.na(.data[[clinvar]])) %>%
  group_by(`Risk Group`, .data[[clinvar]]) %>%
...
g <- ggplot(tmp, aes(x = `Risk Group`, y = count, fill = .data[[clinvar]])) +
  ...
  theme(
    ...
    legend.title = element_blank(),#element_text(size = 20, face = "bold"),
    legend.text = element_text(size = 34, lineheight = 1)
    #legend.width = unit(2, "in"))  # Adjust legend width if necessary
  ) 

# Display the plot
print(g)

ggsave(paste0("OP/Pipeline OP5/Train OP/Images/TCGA_full_log2counts - ", clinvar, "2.tiff"), plot = last_plot(), width = 1500/72, height = 1000/72, dpi = 300)
...
# Look at relationship of prior glioma and intervention with risk score
names(df)

dat <- df #full_new_tcga_nom

tmp1 <- subset(dat, dat$`WHO CNS5` == 'Astrocytoma, IDH-mutant')
tmp1 <- data.frame(tmp1$`WHO CNS5`, tmp1$Risk.Score)

tmp <- dat[,c("WHO CNS5", "Prior glioma", "Primary outcome", "Risk Score", "Risk Group")]
tmp$Intervention <- ifelse(dat$Intervention == "Chemotherapy + Other Tx", "Chemo + Other Tx",
                           ifelse(dat$Intervention == "Chemotherapy only", "Chemo only",
                                  ifelse(dat$Intervention == "RT + Chemotherapy only", "RT + Chemo",
                                         ifelse(dat$Intervention == "RT", "RT only",
                                                ifelse(dat$Intervention == "Other Tx", "Other Tx", NA)))))

test_tmp <- dat[dat$Intervention != c("Other Tx", "Chemo + Other Tx"), ]
sum(is.na(test_tmp))

# Strat_intervention: open a TIFF device to save the plot ####
tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_train_log2counts - Strat_intervention_test.tiff",
     width = 12, height = 10, units = "in", res = 300)

p <- ggboxplot(
  data = subset(tmp, !is.na(Intervention)), 
  x = "Intervention", 
  y = "Risk Score",
  color = "Intervention", 
  palette = "lancet",
  ...
  theme(
    ...
    plot.margin = margin(t = 20, r = 20, b = 21, l = 21, unit = "pt")
  ) +
  labs(x = "Intervention", y = "Risk Score")
...
# Prior glioma diagnosis: open a TIFF device to save the plot ####
tiff(filename = "OP/Pipeline OP5/Train OP/Images/TCGA_train_log2counts - Strat_prior_glioma.tiff",
     width = 12, height = 10, units = "in", res = 300)

p <- ggboxplot(
  data = subset(tmp, !is.na(`Prior glioma`)), 
  x = "Prior glioma", 
  y = "Risk Score",
  color = "Prior glioma", 
  palette = "lancet",
  ...
  labs(x = "Prior Glioma Diagnosis", y = "Risk Score")
...
# Stacked bar to recreate plots from Figure 5 of original submission ####
load("TCGA_summarized_intervention.Rdata")

# merge intervention data with new_tcga_nom
